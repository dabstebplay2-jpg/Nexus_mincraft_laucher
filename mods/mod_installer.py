...
class ModInstaller:
    ...
    def build_incompatible_message(self, version, instance, project_type="mod"):
        return (
            ...
            "Выбери мод, который поддерживает именно эту версию Minecraft и этот loader."
        )
    ...
    def save_installed_index(self, instance, project, version, installed_files, project_type="mod"):
        ...
        record = {
            "project_id": project.get("project_id") or version.get("project_id"),
        ...
        data["mods"] = [
            ...
            if (item.get("project_id"), item.get("slug")) != (record["project_id"], record["slug"])
        ]
...
class ModInstallerWorker(QThread):
    ...
    def run(self):
        try:
            ...
            installed = installer.install_project(self.project, self.instance, project_type=project_type)
            ...
            if self.download_task_id:
                self.download_manager.fail_task(self.download_task_id, str(error), status="Ошибка установки")
            self.failed.emit(str(error), traceback.format_exc())

    def _on_progress(self, value):
        self.progress.emit(int(value))
        if self.download_task_id:
            self.download_manager.update_task(self.download_task_id, progress=int(value))
    ...
    def _update_task(self, **kwargs):
        if self.download_task_id:
            self.download_manager.update_task(self.download_task_id, **kwargs)
